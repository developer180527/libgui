...
impl Atlas {
    ...
    /// Shelf packer. Returns None when full.
...
pub struct Fonts {
    ...
    glyphs: HashMap<(u16, char, u32), Glyph>,
    atlas: Atlas,
    scale: f32,
}
...
impl Fonts {
    pub fn new() -> Self {
        Self { fonts: Vec::new(), glyphs: HashMap::new(), atlas: Atlas::new(2048), scale: 1.0 }
    }
    ...
    fn line(&self, font: FontId, px: f32) -> (f32, f32) {
        ...
        m.map(|m| (m.ascent, m.descent)).unwrap_or((px * 0.8, -px * 0.2))
    }
    ...
    /// after the last one: `len == chars + 1`. Matches `draw` exactly.
    ...
    fn glyph(&mut self, font: FontId, ch: char, px: f32) -> Glyph {
        ...
        let mut uv = [0.0; 4];
        if w > 0 && h > 0 {
            let pos = match self.atlas.alloc(w, h) {
                Some(p) => p,
                None => {
                    ...
                    self.atlas.reset();
                    self.glyphs.clear();
                    self.atlas.alloc(w, h).expect("glyph larger than atlas")
                }
            };
