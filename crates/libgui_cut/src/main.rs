//! A host for the video editor: one OS window per dock surface. The main window
...
impl Host {
    fn create_window(&mut self, el: &ActiveEventLoop, dock_id: SurfaceId, title: &str, size: Vec2, inner_pos: Option<Vec2>) -> WindowId {
        ...
        let mut ui = Ui::new(libgui_cut::theme(), FONT).expect("bundled font");
    ...
    fn draw(&mut self, wid: WindowId) {
        ...
        // The edit's own clock: playback and the meters move in real time, so
        // the host — which owns the clock — advances them.
        if w.dock_id == SurfaceId::MAIN {
            self.app.ed.tick(dt);
        }

        let mut platform = PlatformOutput::default();
        // While playing, every frame differs whether or not input arrived.
        if self.app.ed.playing || w.ui.needs_frame_for(&info, w.idle) {
            w.idle = 0.0;
