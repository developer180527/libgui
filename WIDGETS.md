...
| **Numeric field with units and expressions** | 9 | **no** |
...
| Inline validation error on a field | 11 | **no** |
...
|---|---|---|---|
| 1 | Numeric field with units and expressions | Every CAD dimension box. `drag_value` covers the mouse; nothing covers typing `25.4mm` or `3/8"` or `w/2`. | 3–4 d |
...
| 3 | Inline field validation | The other half of data entry: a field that rejects input silently is worse than one that has no validation. | 1–2 d |
